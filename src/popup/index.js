...
document.addEventListener("DOMContentLoaded", () => {
  document
    .getElementById("fxCheck")
    .addEventListener(
      "click",
      (event) => runtime.storage.sync.set({ fx: event.target.checked }),
      { passive: true },
    );
  //Add event listener
  document.getElementById("flex").addEventListener(
    "click",
    (event) => {
      try {
  ...
  document.getElementById("fx").innerText =
    runtime.i18n.getMessage("fxDescription");
